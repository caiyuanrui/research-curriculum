...
Let p̃_σ(x̃|x) = N(x̃; x, σ²I) be the corruption process, and let q_σ(x̃) = ∫ p_data(x) · p̃_σ(x̃|x) dx be the corrupted data distribution (a Parzen density estimate). The DAE learns a reconstruction function r_θ(x̃) that minimizes E[ ‖r_θ(x̃) − x‖² ].

Vincent showed that the optimal reconstruction function r*(x̃) satisfies:

$$
r^*(x̃) − x̃ = σ² · ∇_{x̃} log q_σ(x̃)
$$
...
$$
E[X | X̃ = x̃] = x̃ + σ² · ∇_{x̃} log p_σ(x̃)
$$

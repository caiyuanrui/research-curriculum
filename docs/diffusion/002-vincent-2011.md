...
Let p̃_σ(x̃|x) = N(x̃; x, σ²I) be the corruption process, and let q_σ(x̃) = ∫ p_{\text{data}}(x) · p̃_σ(x̃|x) dx be the corrupted data distribution (a Parzen density estimate). The DAE learns a reconstruction function r_θ(x̃) that minimizes E[ ‖r_θ(x̃) − x‖² ].
